...
if __name__ == "__main__":
    ...
    from WorkingProjects.QM_Team_fromBFF.qubit_measurements.Client_modules.Experiments.mZeroSpanParity import ZeroSpanParity
    ...
    xd = np.linspace(3049.0, 3051.0, 401)   # dx = 0.005 MHz
    dxd = xd[1] - xd[0]
    center_d = 3050.0
    g_d = 0.02

    def _noise(scale=0.01):
        return rng_d.normal(0.0, scale, xd.size)

    # 1. Clean symmetric doublet (sep = 0.1 MHz, equal heights).
    f_lo_t, f_hi_t = 3049.95, 3050.05
    y_doub = (_lorentz(xd, f_lo_t, 1.0, g_d)
              + _lorentz(xd, f_hi_t, 1.0, g_d) + _noise())
    r1 = find_parity_doublet(xd, y_doub, center_d, min_sep_mhz=0.02,
                             prominence_snr=5.0, fit_window_mhz=0.05)
    assert r1["mode"] == "doublet", r1["mode"]
    assert abs(r1["peak_sep"] - 0.1) < 0.01, r1["peak_sep"]
    assert abs(r1["center"] - center_d) < 0.01, r1["center"]

    # 2. Spurious tall spike must NOT break the doublet (fails balance/symmetry).
    y_spk = y_doub.copy()
    y_spk[5] += 5.0   # one-sample spike near the low-frequency edge
    r2 = find_parity_doublet(xd, y_spk, center_d, min_sep_mhz=0.02,
                             prominence_snr=5.0, fit_window_mhz=0.05)
    assert r2["mode"] == "doublet", r2["mode"]
    assert abs(r2["center"] - center_d) < 0.02, r2["center"]

    # 3. Single centered peak -> single mode.
    y_one = _lorentz(xd, center_d, 1.0, g_d) + _noise()
    r3 = find_parity_doublet(xd, y_one, center_d, min_sep_mhz=0.02,
                             prominence_snr=5.0, fit_window_mhz=0.05)
    assert r3["mode"] == "single", r3["mode"]
    assert abs(r3["center"] - center_d) < 0.01, r3["center"]

    # 4. Sub-bin refinement beats the bin grid for off-grid peaks.
    f_lo_o, f_hi_o = 3049.9531, 3050.0467
    y_off = (_lorentz(xd, f_lo_o, 1.0, g_d)
             + _lorentz(xd, f_hi_o, 1.0, g_d) + _noise(0.005))
    r4 = find_parity_doublet(xd, y_off, center_d, min_sep_mhz=0.02,
                             prominence_snr=5.0, fit_window_mhz=0.05, refine=True)
    assert r4["mode"] == "doublet", r4["mode"]
    assert abs(r4["lower"] - f_lo_o) < dxd, (r4["lower"], f_lo_o)
    assert abs(r4["upper"] - f_hi_o) < dxd, (r4["upper"], f_hi_o)

    # 5. Height-imbalanced pair fails the balance gate -> single at the tall peak.
    y_imb = (_lorentz(xd, 3049.95, 1.0, g_d)
             + _lorentz(xd, 3050.05, 0.1, g_d) + _noise(0.005))
    r5 = find_parity_doublet(xd, y_imb, center_d, min_sep_mhz=0.02,
                             prominence_snr=4.0, min_height_balance=0.3,
                             fit_window_mhz=0.05)
    assert r5["mode"] == "single", r5["mode"]
    assert abs(r5["center"] - 3049.95) < 0.01, r5["center"]

    # 6. estimate_noise_floor is robust to a few tall peaks.
    base, sig_n = estimate_noise_floor(y_doub)
    assert abs(base) < 0.05, base
    assert 0.0 < sig_n < 0.1, sig_n

    print("utils.py find_parity_doublet: OK")
