...
Usage
-----
    from WorkingProjects.QM_Team_fromBFF.qubit_measurements.Client_modules\\
...
AUTO_COHERENCE_PARAMS = {
    ...
    # Set run_sweet_spot_search=False (default) to skip Stage 1 entirely: the
    # run then starts from the qubit_params frequency without any two-tone spec
    # sweep or yoko voltage stepping (sweet_spot_voltage is reported as None).
    ...
    "run_sweet_spot_search": False,  # run Stage 1 (off by default)
...
def run_auto_coherence(soc, soccfg, config, outerFolder,
    ...
    ----------
    soc, soccfg
        QICK hardware proxies (from makeProxy_RFSOC_147()).
    config : dict
    ...
    # ── Stage 1: Sweet-spot search (optional) ────────────────────────────────
    if params.get("run_sweet_spot_search", False):
        ...
        # Skip the two-tone spec sweep / yoko voltage stepping; start from the
        # qubit_params frequency as-is.
        qubit_freq = qubit_freq_init
        sweet_spot_voltage = None
        peak_info = None
        log.append("=" * 60)
        log.append("STAGE 1 – Sweet-spot search skipped (run_sweet_spot_search=False)")
        log.append("=" * 60)
        log.append(f"  Using qubit_frequency: {qubit_freq} MHz")
